...
interface MediaPersonaSelectionStepProps {
  selectedPersonaIds: string[];
  onPersonaToggle: (personaId: string) => void;
}

export const MediaPersonaSelectionStep: React.FC<
  MediaPersonaSelectionStepProps
> = ({ selectedPersonaIds, onPersonaToggle }) => {
  ...
  const handleClearSelection = () => {
    selectedPersonaIds.forEach((id) => onPersonaToggle(id));
  };
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <h3 className="text-lg font-semibold">Select Personas</h3>
        <p className="text-muted-foreground text-sm">
          Choose the personas that will be used to evaluate the media content
        </p>
      </div>
                  ...
                  <CommandGroup>
                    {personaData?.items?.map((persona) => (
                      <CommandItem
                        key={persona._id}
                        value={persona.name}
                        onSelect={() => handlePersonaToggle(persona._id)}
                        className="flex items-center space-x-2"
                      >
                        <Checkbox
                          checked={selectedPersonaIds.includes(persona._id)}
                          onChange={() => {}}
                        />
      ...
      {selectedPersonaIds.length > 0 && (
        <div className="flex items-center space-x-2">
          ...
          <Badge variant="secondary">{selectedPersonaIds.length}</Badge>
        </div>
      )}
